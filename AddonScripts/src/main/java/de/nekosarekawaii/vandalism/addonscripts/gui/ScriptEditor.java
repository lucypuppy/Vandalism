...
import de.nekosarekawaii.vandalism.addonscripts.parse.info.impl.StringScriptInfo;
import de.nekosarekawaii.vandalism.addonscripts.base.Script;
import de.nekosarekawaii.vandalism.addonscripts.parse.info.impl.BooleanScriptInfo;
...
public class ScriptEditor implements MinecraftWrapper {
    ...
    private static String getExampleCode() {
        ...
        for (final Placeholders placeholders : Placeholders.values()) {
            exampleCodeBuilder.append("\t\t").append(ScriptParser.VARIABLE_CHAR).append(placeholders.name().toLowerCase()).append(ScriptParser.VARIABLE_CHAR).append(" - ").append(placeholders.getDescription()).append('\n');
        }
        exampleCodeBuilder.append("\n\n\n");
    ...
    public ScriptEditor(final File scriptFile, final String code, final boolean rename) {
        ...
        for (final Placeholders placeholders : Placeholders.values()) {
            keywords.add(placeholders.name().toLowerCase());
        }
        ...
        for (final Placeholders placeholders : Placeholders.values()) {
            preprocIdentifierMap.put(placeholders.name().toLowerCase(), placeholders.getDescription());
        }
    ...
    public boolean canBeSaved() {
        ...
        if (this.rename) {
            final File[] files = AddonScripts.getInstance().getScriptManager().getDirectory().listFiles();
            if (files != null) {
            ...
            return !this.scriptFile.exists() || !this.scriptName.get().equals(this.originalScriptName);
        }
        return true;
    }
    ...
    public void save() {
        try {
            ...
            if (this.rename) {
                ...
                this.scriptFile = new File(AddonScripts.getInstance().getScriptManager().getDirectory(), this.scriptName.get() + ScriptParser.SCRIPT_FILE_EXTENSION);
            ...
            printerWriter.close();
            AddonScripts.getInstance().getScriptManager().loadScriptFromFile(this.scriptFile, true);
            this.lastScriptFileModification = this.scriptFile.lastModified();
    ...
    public void render() {
        ...
        if (ImGui.beginTabItem(this.scriptFile.getName() + (this.isReadOnly() ? " (Read-Only)" : "") + "##scriptstab" + this.originalScriptName + "editor")) {
            ...
            final Script script = AddonScripts.getInstance().getScriptManager().getList().stream().filter(s -> s.getFile().getName().equalsIgnoreCase(this.scriptFile.getName())).findFirst().orElse(null);
            if (script != null) {
                final UUID uuid = script.getUuid();
                if (AddonScripts.getInstance().getScriptManager().isScriptRunning(uuid) || (!this.canBeSaved() && this.scriptFile.exists() && this.scriptFile.length() > 0 && this.mc.player != null)) {
                    ImGui.sameLine();
                    if (ImGui.button((AddonScripts.getInstance().getScriptManager().isScriptRunning(uuid) ? "Kill" : "Execute") + "##scriptsexecuteorkillin" + this.originalScriptName + "editor", buttonWidth, buttonHeight)) {
                        if (AddonScripts.getInstance().getScriptManager().isScriptRunning(uuid)) {
                            AddonScripts.getInstance().getScriptManager().killRunningScript(uuid);
                        } else AddonScripts.getInstance().getScriptManager().executeScript(uuid);
                    }
                }
            }
