...
public class ScriptArgumentType implements ArgumentType<Script> {
    ...
    public Script parse(final StringReader reader) throws CommandSyntaxException {
        ...
        final Script script = AddonScripts.getInstance().getScriptManager().getByName(argument, true);
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(final CommandContext<S> context, final SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(AddonScripts.getInstance().getScriptManager().getList().stream().map(Feature::getName), builder);
    }
