...
public class CustomHUDConfig extends ValueableConfig {
    ...
    public JsonObject save() throws IOException {
        ...
        for (final HUDElement hudElement : hudElements) {
            ...
            elementObject.addProperty("x", hudElement.x);
            elementObject.addProperty("y", hudElement.y);
    ...
    public void load(final JsonObject jsonObject) throws IOException {
        ...
        for (final HUDElement hudElement : hudElements) {
            ...
            if (elementObject != null) {
                if (elementObject.has("x") && elementObject.has("y")) {
                    hudElement.x = elementObject.get("x").getAsInt();
                    hudElement.y = elementObject.get("y").getAsInt();
                } else {
                    Vandalism.getInstance().getLogger().error("HUD Element " + hudElement.getName() + " has no position in the config!");
                }
