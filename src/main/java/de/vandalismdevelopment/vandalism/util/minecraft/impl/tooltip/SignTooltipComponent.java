...
public abstract class SignTooltipComponent<M extends Model> implements TooltipComponent, ITooltipData, MinecraftWrapper {
    ...
    public static Optional<TooltipData> fromItemStack(final ItemStack stack) {
        if (stack.getItem() instanceof HangingSignItem signItem) {
            ...
            if (nbt != null) {
                return Optional.ofNullable(fromTag(AbstractSignBlock.getWoodType(block), nbt, true));
            }
        } else if (stack.getItem() instanceof SignItem signItem) {
            final Block block = signItem.getBlock();
            final NbtCompound nbt = BlockItem.getBlockEntityNbt(stack);
            if (nbt != null) {
                return Optional.ofNullable(fromTag(AbstractSignBlock.getWoodType(block), nbt, false));
            }
        }
    ...
    public static SignTooltipComponent<?> fromTag(final WoodType type, final NbtCompound nbt, final boolean hanging) {
        ...
        if (nbt.contains("front_text")) {
            front = SignText.CODEC.parse(NbtOps.INSTANCE, nbt.getCompound("front_text")).resultOrPartial(s -> {
            }).map(SignTooltipComponent::parseLines);
        }
        if (nbt.contains("back_text")) {
            back = SignText.CODEC.parse(NbtOps.INSTANCE, nbt.getCompound("back_text")).resultOrPartial(s -> {
            }).map(SignTooltipComponent::parseLines);
        }
    ...
    private OrderedText[] getOrderedMessages() {
        return this.getText().getOrderedMessages(this.mc().shouldFilterText(), Text::asOrderedText);
    }
    ...
    public void drawItems(final TextRenderer textRenderer, final int x, final int y, final DrawContext graphics) {
        ...
        final SignText signText = this.getText();
        final int outlineColor = SignBlockEntityRenderer.getColor(signText);
        final int light = LightmapTextureManager.MAX_LIGHT_COORDINATE;
        if (signText.isGlowing()) {
            for (int i = 0; i < messages.length; i++) {
                final OrderedText text = messages[i];
                final int textX = (int) (45 - textRenderer.getWidth(text) / 2.f);
                final int textY = i * 10;
                final Matrix4f matrix = matrices.peek().getPositionMatrix();
                final VertexConsumerProvider.Immediate vertexConsumers = graphics.getVertexConsumers();
                textRenderer.drawWithOutline(text, textX, textY, signColor, outlineColor, matrix, vertexConsumers, light);
            }
        } else {
            for (int i = 0; i < messages.length; i++) {
                final OrderedText text = messages[i];
