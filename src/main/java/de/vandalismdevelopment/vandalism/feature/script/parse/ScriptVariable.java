package de.vandalismdevelopment.vandalism.feature.script.parse;
...
public enum ScriptVariable {
    ...
    USERNAME(() -> MinecraftClient.getInstance().session.getUsername(), descriptionBuilder -> {
    ...
    ScriptVariable(final Supplier<String> scriptReplacement, final Function<StringBuilder, StringBuilder> scriptVariableDescription) {
    ...
    public static String applyReplacements(String text) {
        if (!text.contains(ScriptParser.VARIABLE_CHAR)) return text;
        for (final ScriptVariable variable : values()) {
            final String sequence = ScriptParser.VARIABLE_CHAR + variable + ScriptParser.VARIABLE_CHAR;
