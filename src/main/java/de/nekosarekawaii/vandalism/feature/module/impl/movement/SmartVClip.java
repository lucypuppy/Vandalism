...
public class SmartVClip extends AbstractModule implements KeyboardInputListener {
    ...
    private final KeyBindValue upKey = new KeyBindValue(this, "Up Key",
            "The key that needs to be pressed to go upward", GLFW.GLFW_KEY_UP);

    private final KeyBindValue downKey = new KeyBindValue(this, "Down Key",
            "The key that needs to be pressed to go downward", GLFW.GLFW_KEY_DOWN);

    private final IntegerValue maxScanRange = new IntegerValue(this, "Max Scan Range",
            "The maximum range to port to", 9, 3, 100);

    public SmartVClip() {
        super("SmartVClip", "A automatic vclip module", Category.MOVEMENT);
    }
    ...
    public void onKeyInput(long window, int key, int scanCode, int action, int modifiers) {
        ...
        if (upKey.isPressed() || downKey.isPressed()) {
            ...
            for (int i = 1; i < maxScanRange.getValue(); i++) {
                final BlockPos safeBlockPos = oldPos.add(0, upKey.isPressed() ? i : (-(i + 1)), 0);
                ...
                if (safeBlock.isSolidSurface(mc.world, safeBlockPos, mc.player, Direction.UP) &&
                        airBlock1State.isAir() && airBlock2State.isAir()) {
                    mc.player.updatePosition(safeBlockPos.getX() + 0.5, safeBlockPos.getY() + 1, safeBlockPos.getZ() + 0.5);
                    break;
                }
            }
        }
