...
//Add sparks
particles += (1.0 - smoothstep(PARTICLE_SIZE * 0.6, PARTICLE_SIZE * 3.0, dist)) * (sparkColor * 1.5);

//Add bloom
particles += pow((1.0 - smoothstep(0.0, PARTICLE_SIZE * 6.0, distBloom)) * 1.0, 3.0) * (bloomColor * 0.8);
...
vec3 smoke = smokeIntensity * (smokeColor * 0.8) * 0.8 * vignette;
...
vec3 col = particles + smoke + (smokeColor * 0.8) * 0.02;
col *= vignette;
